...
class ComplexRetriever(BaseRetriever):
    ...
    def retrieve_top_k(self, namespace: str, top_k: int, query: str):
        ...
        seen = set()
        cleaned_retrieval = [doc for doc in all_retrieval if not (doc in seen or seen.add(doc))]
    ...
    def _compute_hypothesis_embedding(self, query: str):
        ...
        hypothesis_answer = self.llm_client.models.generate_content(model = self.model,
                                                                    contents = prompt)
        if hypothesis_answer.text is None:
            raise ValueError("Unable to generate the hypothesis answer")
    ...
    def _compute_sub_query_embedding(self, query: str):
        ...
        sub_query = self.llm_client.models.generate_content(model = self.model,
                                                            contents = prompt)

        if sub_query.text is None:
            raise ValueError("Unable to generate subquery")

        return self.embedder.embed(str(sub_query.text))
